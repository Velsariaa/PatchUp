...
import styles from '../styles/signUpPageStyles'; // Your custom styles
...
export default function SignUp() {
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      const userRef = doc(firestore, 'users', email); // User's email will be the document ID
      await setDoc(userRef, {
        firstName,
        lastName,
        email,
        mobile,
        uid: user.uid, // Optionally save the Firebase user ID
      });

      setErrorMessage('');
      console.log('User successfully signed up and data saved to Firestore!');
      navigation.navigate('SignIn');

      // Optionally navigate to a different screen after successful sign-up
      // For example, navigate to a welcome or login screen

    } catch (error) {
      // Handle Firebase errors (e.g., email already in use, weak password, etc.)
      ...
      >
        <ScrollView>
          <SafeAreaView style={styles.innerContainer}>
            ...
            <Text>
              Already have an account? <Text style={{ color: '#01B3D7' }}>Sign In</Text>
            </Text>
